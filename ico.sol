...
contract ICO_CONTRACT is Ownable {
    ...
    address public ico_coin;
    bool public icoStatus;

    constructor(address _Admin_Wallet, address _ico_coin) {
    ...
    function change_ico_coin(address _new_address) public onlyOwner {
        ico_coin = _new_address;
    }

    function get_coin_bal () public view returns (uint256){
      return ICO_TK.balanceOf(address(this));
    }
    ...
    IERC20 ICO_TK =
        IERC20(
            address(ico_coin) // TGC Token address
        );
        ...
        public
    {
        ...
        if (_Referal_Id == 0) {
            if (user_id == 0) {
                ...
                ICO_TK.transfer(msg.sender, Usdt_Amount * current_Price);
            ...
            if (user_id == 0) {
                ...
                REFRAL_EARNING[referal_address] = referal_amount * current_Price;
                ICO_TK.transfer(msg.sender, Usdt_Amount * current_Price);
                totalRaisedAmount = totalRaisedAmount + Usdt_Amount;
                Create_referal_url(msg.sender);
            } else {
        ...
        public
    {
        ...
        if (_Referal_Id == 0) {
            if (user_id == 0) {
                usdt.transferFrom(msg.sender, Admin_Wallet, Usdt_Amount);
                ICO_TK.transfer(msg.sender, Usdt_Amount * (current_Price + ExtraReward));
                totalRaisedAmount = totalRaisedAmount + Usdt_Amount;
                Create_referal_url(msg.sender);
            } else {
                usdt.transferFrom(msg.sender, Admin_Wallet, Usdt_Amount);
        ...
        public
    {
        ...
        if (_Referal_Id == 0) {
            if (user_id == 0) {
                ...
                ICO_TK.transfer(msg.sender, busd_Amount * current_Price);
        ...
        public
    {
        ...
        if (_Referal_Id == 0) {
            if (user_id == 0) {
                Busd.transferFrom(msg.sender, Admin_Wallet, busd_Amount);
                ICO_TK.transfer(msg.sender, busd_Amount * (current_Price + ExtraReward));
                totalRaisedAmount = totalRaisedAmount + busd_Amount;
                Create_referal_url(msg.sender);
            } else {
                Busd.transferFrom(msg.sender, Admin_Wallet, busd_Amount);
    ...
    function Private_sale(uint256 TGC_Ammount, address Reciver) public onlyOwner {
        ICO_TK.transfer(Reciver, TGC_Ammount * 10**18);
    }
